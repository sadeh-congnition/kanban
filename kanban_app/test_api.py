...
def test_get_project_tags(api_client):
    project = baker.make(Project)
    tag = baker.make(Tag, project=project, name="Bug")
...
def test_get_board_columns(api_client):
    ...
    col = baker.make(Column, board=board, name="To Do")
...
def test_create_column(api_client):
    board = baker.make(Board)
    response = api_client.post(f"/api/boards/{board.id}/columns", {"name": "In Progress"})
    assert response.status_code == 200
...
def test_update_task_tags(api_client):
    ...
    response = api_client.post(f"/api/tasks/{task.id}/tags", {"tags": [tag1.id, tag2.id]})
